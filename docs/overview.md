...
A turn that reached the model records what it cost when it ends: the tokens it was sent, how many of those the model had already cached, the tokens it wrote back, and what that came to in money, as the model reported them rather than as an estimate. It records the model it ran on as well, since an agent's model is whatever it is today and a past turn cost what it cost on the model that took it, and how many requests it took to finish, since a turn that calls a tool asks the model again with everything it has learned, and a long turn is therefore many requests rather than one large one. A turn recorded before AgentOS kept that has none, and is counted apart rather than guessed at. A turn canceled or failed before an answer records nothing, since nothing was spent that anyone can name.
...
An agent can be deleted, which removes it from the workspace and nothing more. Nothing is kept to say what it was called, so every entry that points at its id reads as unknown from then on, while staying exactly as it was written. Deleting is refused while the agent is acting, so no turn loses its agent halfway.
...
Script tools are defined by the user in the app, with no AgentOS release needed, so every installation carries its own. A script tool is a JavaScript function: it receives an input object that conforms to inputSchema, plus an env object holding only the workspace env keys the tool declares, and returns an output object that must conform to outputSchema. Around it is the machine's own environment, minus whatever AgentOS runs itself under, so a tool and the commands it spawns never behave differently because of how the app happened to be started. When the function spawns a command, input values are passed as arguments and never interpolated into a shell string.
...
A script tool can be deleted. The permission every agent held for it goes with it, since a permission for a tool the workspace no longer has is nothing anyone could read or act on. Its past calls stay in the threads that made them, and a workflow step naming it fails when a run reaches that step, exactly as one naming a tool that was never written does.
...
A source can be deleted once nothing mounts it: while any conversation of the workspace has it mounted, deleting is refused and names the conversations that do, so a mount is never pulled out from under a conversation that is using it. What goes with a git source is the workspace's own clone of the repository, and with that every commit that was never pushed; what is untouched is the remote, and a directory source's directory, which AgentOS does not own. Past mount calls stay in the threads that made them.

A source also carries a description, which the user writes and rewrites: what the data is, and where in it the parts worth knowing about are, such as which directory of a company repository holds the manifests. A mount hands it back in its result, so whoever mounted the source is told what the workspace knows about it rather than having to go looking. A source may have none, since most need no explaining.
...
Memories are the workspace's own, one pool shared by every agent in it and reachable from no other workspace. A title is unique, compared without regard to case, so writing onto a title already taken is refused and names what is there: a memory that is wrong is corrected rather than duplicated. Tags are names rather than prose, lower case letters, digits, hyphens and underscores, since a tag is matched exactly against what an agent carries; a memory may carry no tags at all. A body is at most WORKSPACE_MEMORY_LIMIT characters, 2000 where the workspace does not say otherwise, because what a workspace knows is a paragraph rather than a runbook, and that limit is what keeps both carrying and searching affordable.
...
A conversation archives and a workspace is deleted, and neither can be undone. Archiving a conversation closes it for good: there is no unarchive, its thread stays readable forever, and its sandbox and checkouts are removed, so work that was never pushed is gone. Archiving is available at any moment, blocked or not: it cancels whatever call is pending or running, exactly as if the user had canceled the turn, and those entries keep their canceled status in the closed thread. Because none of it can be undone, archiving asks first, and says what goes with the conversation.
...
What a workspace holds is removed one piece at a time, each where it is written: an agent in the agents pane, a script tool in the tools pane, a mount source in the sources pane, a workflow in the workflows pane, a memory in the memories pane. Removal exists because something wrong keeps costing while it stays: a workflow keeps offering to run, a tool keeps being callable, a memory is handed to agents at the start of every turn. So removing one is a correction rather than tidying, and what it corrects is what the workspace can do next.

What removal never corrects is what the workspace did. Entries are never rewritten, so a thread that called a tool, mounted a source or carried a turn by an agent reads afterwards exactly as it read before, save that a deleted agent is named unknown, since nothing is kept to say what it was called. Removal is refused while the thing is in use, which is a source a conversation has mounted and an agent that is acting, and because none of it can be undone, each asks first and says what goes with it.
...
Built-in tools carry no timestamps: they are part of the app, not records. All timestamps are ISO 8601. Together with append-only conversations, every action in AgentOS is traceable to a moment in time, for as long as its workspace exists. What a line holds is never rewritten, but a line of a kind this version no longer knows is skipped when the thread is read, so it counts for nothing and shows as nothing: the file keeps it, the app has forgotten how to read it.

The scope is deliberately actions, not definitions. Agent prompts, permissions, script tool code, and memories are edited in place without version history: a past entry tells you exactly what happened and when, while the agent or tool it points at is whatever that definition is today, or nothing at all once it has been removed.

## Built-in tools

Most built-in tools act on the conversation's sandbox and respect read-only mounts; the git remote tools reach exactly as far as the mount's remote, and the mount tools reshape the sandbox itself. The tools for building tools, the tools for building agents, the tools for building workflows and the memory tools are the exception: they act on the workspace rather than on the sandbox, on what it can do, on who does it, in what order it does it, and on what it knows. Two act on neither: ask_user asks the person at the keyboard, and workflow_result hands back what a step was asked for. One acts on the conversation the call happens in: rename_conversation, since its title is the one thing about a conversation a tool can change. The git tools name the mount they target by its sandbox path. Running anything is authorized per command, by defining a script tool for it, and no agent is handed a general command runner as a matter of course.

The exception is the work of building tools, which cannot be done blind: finding out what a command does, what its options are and what shape its output takes is how a tool gets written at all. So there are three tools for that work, define_tool, update_tool and run_command, and they are ordinary tools an agent is granted or not. Granting them is the largest permission in AgentOS and the doc is blunt about why: a tool is trusted code with no boundary around what it spawns, so an agent that may define a tool can already run anything, and withholding a command runner from it would only push the same power into a code blob that is harder to read. Granted as ask, every command and every tool arrives as a pending call carrying the exact command line, or the exact code, for the user to read before it happens. Building ends with trying: a tool an agent defines is granted to that agent as ask the moment it exists, so testing it is not a trip through the agent pane. Like every permission, the grant is read as a turn starts, so the definer calls its new tool from its next turn on. When the user defines a tool themselves nothing is granted, since permissions do not apply to the user. Reading what the workspace already has carries none of that weight and is its own pair: list_tools says what it can do and read_tool reads one whole, both kinds included, since a built-in is a name a definer may not take and a permission a caller may name.

Building agents is the same work one level up, and carries the same weight. list_agents and read_agent say who the workspace has and how one is configured, create_agent adds one, and update_agent changes what is already there. An agent that may write agents can grant permissions it was never given itself, and may rewrite its own prompt and its own permissions, both of which are allowed rather than quietly refused: the guard is the pending call, which carries the exact prompt and the exact permission list for the user to read before that agent exists or changes. Permissions are named there by tool name rather than by tool id, since ids are what the workspace generated and names are what a caller can know, and a name matching no tool of the workspace refuses the call. The model is named as one of the models AgentOS offers, anything else refuses the call, and a create that leaves it out gets the default. There is no tool for removing an agent, nor one for removing a script tool or a mount source: an agent may add these and change them, and only the user removes them, in the pane where each is written. A workflow and a memory are the exception, since both keep costing while they are wrong, one by offering to run and the other by being handed to every turn.
...
- list_tools: list the tools of the workspace, built-in and script alike, with what each one is for
- read_tool: read one tool whole, naming it by name, its schemas included and a script tool's code with them
...
- A tool call is one line, naming the tool and what it acted on, with what became of it at the right, which for a tool AgentOS knows nothing particular about is how much it returned, such as the rows of a table. Opening the line shows its input and output, each field read as its schema declares it. A call waiting on a decision is open already, since its input is what is being approved, and one still running is open too, since it can be stopped.
...
- Messages and tool calls offer copy to clipboard: a message copies its text, a tool call its input and output as JSON. Turn entries have nothing to copy. The viewer copies the path of the file it has open, which is the path a call or a slash command names it by.
...
- The window is two panes, the sidebar with the workspace picker and the conversation list, and the thread beside it, and a third while a file, a run, or what a turn sends is open beside it. One of those at a time: opening any of them closes whichever was there. The sidebar slides out of the way and back, and stays as you left it.
...
- The viewer is the conversation's sandbox: a tree of it down one side, and whatever is open beside that. A folder opens and closes, and is read the first time it is opened rather than all of it at once; folders come before files, each in name order. Picking a file in the tree opens it, and a conversation with no sandbox yet has nothing to show.
- The conversation's header opens the viewer on the sandbox with nothing picked yet, and closes it again when it is already showing the sandbox.
- A tool call opens in the viewer what it names, the field its schema declares a path, as it is now: markdown rendered, text as its source with its lines numbered and coloured for whatever the name says the file is, a directory as the names in it, anything else described rather than drawn. A file too large to read comfortably is shown up to a limit and says so. The tree opens down to that file and marks it, so what a call touched is shown where it sits.
- The viewer is as wide as you drag it, and stays that width next time.
- The viewer never edits. Work on a file happens through the conversation, so that every change is a tool call somebody can read; a change made beside the thread would be a change nobody recorded. It follows the file it shows: a later call touching that path refreshes it, and a file that has since been deleted says so. The tree follows the conversation too, so what its calls add and remove appears there as they settle.
- A conversation's header names it, with a way to rename it while it is open and a closed lock once it is archived. A rename by an agent reaches the header and the sidebar as soon as its call settles, rather than waiting for the turn it happened in to end. Beneath that the header carries what the conversation is bound to: what it has mounted, a git mount naming its source with the branch and commit it currently sits on, the agents that have taken part in it, and its sandbox, which opens in the file manager.
...
- Conversations, agents, script tools, workflows, mount sources, memories, usage and env each open in a pane that replaces the thread.
- The tools pane lists a workspace's script tools in name order, since a tool is looked for by the name it is called by rather than by when it happened to be written. A tool is deleted there, confirming first and saying that the agents holding it lose the permission while its past calls stay in the threads that made them.
...
- The sources pane writes a source's description when it is added and rewrites it in place afterwards, which is the one thing about a source that changes after it exists. A source is deleted there, confirming first and saying what goes with it; one a conversation still mounts is refused, naming the conversations that mount it.
- The usage pane says what the workspace has spent: by model, by agent and by conversation, each with the turns it took, what was sent, how much of that the model had already cached, what came back and what it cost. A conversation listed there opens. Turns that recorded nothing are not counted as free and do not appear, and a turn whose model or agent the workspace can no longer name is counted as unknown rather than guessed at or dropped.
- Above those, a bar for each of the last thirty days, empty days included, since a gap says as much as a spike. The bars read as money or as tokens sent, whichever is chosen, and a bar names its day and its figure.
...
- An Electron app, TypeScript throughout; the UI is React with shadcn/ui components, and the code boxes of the tool editor are CodeMirror. Everything runs and is stored locally, which is why each machine carries its own workspaces and tools.
- Agents run on the Claude Agent SDK: a turn is a query() run with the agent's model and systemPrompt, tools are exposed through an in-process MCP server built with tool() and createSdkMcpServer(), every granted tool is passed as allowedTools so the SDK never rules on a call itself, and each tool records its own call into the thread and waits there for the user when it asks. The SDK's own tools are switched off and the machine's MCP configuration is ignored, so an agent has nothing but what the workspace grants it: what another MCP server on the machine offers is not its to see, and defining it would show the agent tools nobody granted and charge every turn for them.
